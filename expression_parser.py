...
class Lexer:
    ...
    def __next__(self):
        ...
        if self.current < len(self.data):
            ...
            # added code    
            alo = self.func_re.match(self.data[self.current - 1 :])
            if alo is not None:
               self.current += alo.end() - 1
               return (Lexer.FUNC, alo.group())
            match = self.func_re.match(self.data[self.current - 1 :])
...
def parse_T(data):
    ...
    return F if T_prime is None else T_prime(F)
...
def parse_T_prime(data):
    ...
    if token == Lexer.OPERATOR and operator in "*/^":
        ...
        if operator == "*":
            return partial(multiply, y = F)
        elif operator == "/":
            return partial(multiply, y = 1/F)
        else:
            return partial(pow_2, y = F)
    data.put_back()
    return None
