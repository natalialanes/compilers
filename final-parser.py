...
class Lexer:
    ...
    def peek(self):
        if self.current < len(self.data):
            ...
            if char == "=":
                return Lexer.VARIABLE, char, current
...
def parse_N(data):
    ...
    if token not in [Lexer.ID]:
        data.current = last_current
        return parse_E(data)
    else:
        token, value = next(data)
        if value == '=':
            return parse_B(data, identifier)
        else:
            data.current = last_current
            return parse_E(data)


def parse_B(data, id_name):
    symbol_table[id_name] = Symbol(parse_E(data), Lexer.ID)
    return True
...
def parse_F(data):
    ...
    if token == Lexer.ID:
        data.current = last_current
    if token == Lexer.FUNC or Lexer.ID:
...
if __name__ == "__main__":
    expressions = [
        ...
        #Logarithm test
        ("log(120)", math.log(120)),
        ("log(30) + 13", math.log(30) + 13),
        ("test = log(100)", True),
        ("25 + test", 25 + math.log(100))
    ]
